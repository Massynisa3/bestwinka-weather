"""
generate.py — Bestwinka weather dashboard (Polish)
...
# ── Fetch ──────────────────────────────────────────────────────────────────
URL = "https://wttr.in/Bestwinka?lang=pl&format=j1"
if len(sys.argv) > 1:
    ...
    for attempt in range(1, 4):
        try:
            ...
            print(f"Próba {attempt} nieudana: {e}")
            if attempt < 3:
                time.sleep(5)
    else:
        raise SystemExit("wttr.in niedostępny po 3 próbach")

cur  = data["current_condition"][0]
days = data["weather"]  # 3 days

# ── Polish day abbreviations ───────────────────────────────────────────────
_PL_DAY = {"MON": "PON", "TUE": "WT", "WED": "ŚR", "THU": "CZW", "FRI": "PT", "SAT": "SOB", "SUN": "NIE"}

def day_abbr(date_str):
    en = datetime.strptime(date_str, "%Y-%m-%d").strftime("%a").upper()
    return _PL_DAY.get(en, en)
...
def slot_note(rain, wind, temp, part):
    ...
    if rain == 100:               return "Mokro. Zachowaj ostrożność."
    if rain >= 60 and wind >= 20: return "Deszcz prawdopodobny. Wietrznie."
    if rain >= 60:                return "Deszcz prawdopodobny."
    if temp < 8 and rain == 0:    return "Chłodny poranek. Ubierz się warstwowo."
    if rain == 0 and wind >= 20:  return "Sucho, ale wietrznie."
    if rain == 0 and wind < 10:
        notes = {"morning":   "Przyjemny poranek. Lekki wiatr.",
                 "afternoon": "Dobry czas na zewnątrz.",
                 "evening":   "Miły wieczór. Dobra widoczność."}
        return notes[part]
    notes = {"morning":   "Chłodno i sucho. Pochmurnie.",
             "afternoon": "Komfortowo. Lekki wiatr.",
             "evening":   "Spokojny wieczór. Suche warunki."}
    return notes[part]

def fmt_time_ampm(t):
    return datetime.strptime(t.strip(), "%I:%M %p").strftime("%H:%M")

_PART_PL  = {"morning": "rano", "afternoon": "południe", "evening": "wieczór"}
_PART_CAP = {"morning": "Rano", "afternoon": "Południe", "evening": "Wieczór"}
...
for di, day in enumerate(days):
    for part in PARTS:
        ...
        slots.append({
            ...
            "part":    part,
            "temp":    int(h["tempC"]),
            "desc":    translate_desc(h["weatherDesc"][0]["value"].strip()),
...
if any(r > 60 for r in rains):
    conf_pct  = 80
    conf_note = "Wysoka pewność dla okresów suchych.<br>Timing opadów umiarkowanie pewny."
elif all(r <= 30 for r in rains):
    conf_pct  = 92
    conf_note = "Wysoka pewność dla suchej i stabilnej pogody."
else:
    conf_pct  = 84
    conf_note = "Okresy suche pewne; okna deszczowe mniej przewidywalne."
...
if wet_slots:
    ...
    if last_wet is ws:
        ins_rain = f"Wysokie {day_name} {_PART_PL[ws['part']]}"
    else:
        ins_rain = f"Wysokie {day_name} {_PART_PL[ws['part']]}&#x2013;{_PART_PL[last_wet['part']]}"
else:
    ins_rain = "Niskie przez wszystkie dni"

max_wind_slot = max(slots, key=lambda s: s["wind"])
wlvl = "Silny" if max_wind_slot["wind"] >= 30 else "Umiarkowany" if max_wind_slot["wind"] >= 15 else "Słaby"
ins_wind = f"{wlvl}, najsilniejszy {day_abbr(max_wind_slot['date'])} {_PART_PL[max_wind_slot['part']]}"

dry_mild = [s for s in slots if s["rain"] == 0 and 10 <= s["temp"] <= 20 and s["wind"] < 20]
if len(dry_mild) >= 2:
    a, b = dry_mild[0], dry_mild[1]
    ins_best = f"{day_abbr(a['date'])} {_PART_PL[a['part']]} &amp; {day_abbr(b['date'])} {_PART_PL[b['part']]}"
elif dry_mild:
    a = dry_mild[0]
    ins_best = f"{day_abbr(a['date'])} {_PART_PL[a['part']]}"
else:
    ins_best = "Mało okien pogodnych"

morning_temps = [s["temp"] for s in slots if s["part"] == "morning"]
ins_comfort   = f"Chłodne poranki ({min(morning_temps)}&#x2013;{max(morning_temps)}&#xB0;C), łagodne południa"

if wet_slots:
    ins_watch = f"Mokre drogi {day_abbr(wet_slots[-1]['date'])} {_PART_PL[wet_slots[-1]['part']]}"
elif min(morning_temps) < 5:
    cold_day = [s for s in slots if s["part"] == "morning" and s["temp"] < 5][0]
    ins_watch = f"Ryzyko mrozu {day_abbr(cold_day['date'])} rano"
else:
    ins_watch = "Sprawdź prognozę godzinową"
...
best_time  = f"{day_abbr(best_slot['date'])} {_PART_PL[best_slot['part']]}<br>{PART_HOURS[best_slot['part']]}"
best_why   = f"Sucho, {best_slot['temp']}&#xB0;C, wiatr {best_slot['wind']} km/h"
worst_time = f"{day_abbr(worst_slot['date'])} {_PART_PL[worst_slot['part']]}<br>{PART_HOURS[worst_slot['part']]}"
worst_why_parts = []
if worst_slot["rain"] >= 60: worst_why_parts.append("Deszcz")
if worst_slot["wind"] >= 20: worst_why_parts.append("silny wiatr")
if worst_slot["temp"] < 5:  worst_why_parts.append("temp. bliska zeru")
worst_why = ", ".join(worst_why_parts) if worst_why_parts else f"{worst_slot['rain']}% deszczu, {worst_slot['wind']} km/h"
...
wear_rows = ""
if rainy_days:
    wear_rows += f'''<div class="wear-row"><div class="wear-icon">&#x2602;&#xFE0F;</div><div class="wear-txt"><strong>{" &amp; ".join(rainy_days)}:</strong><br>Weź parasol i nieprzemakalną kurtkę</div></div>'''
if dry_days:
    avg_max = sum(max_temps) / len(max_temps)
    jacket = "Lekka kurtka wskazana" if avg_max < 18 else "Pogoda na koszulkę, kurtka na wieczór"
...
_RATING_PL  = {"GOOD": "DOBRY", "FAIR": "ŚREDNI", "POOR": "ZŁY"}
_RATING_CLS = {"GOOD": "ag", "FAIR": "af", "POOR": "ap"}

def rating_badge(r, note=""):
    txt = f"{_RATING_PL[r]} ({note})" if note else _RATING_PL[r]
    return f'<span class="{_RATING_CLS[r]}">{txt}</span>'
...
hike_note = "Sob rano" if any(s["temp"] < 8 and s["part"] == "morning" and day_abbr(s["date"]) == "SOB" for s in slots) else ""
...
drive_note = f"{day_abbr(wet_slots[-1]['date'])} {_PART_PL[wet_slots[-1]['part']].title()}" if wet_slots else ""

act_rows = f"""
<div class="act-row"><span class="act-name">&#x1F6B6; Spacer / Rekreacja</span>{rating_badge(walk_r)}</div>
<div class="act-row"><span class="act-name">&#x1F6B4; Rower</span>{rating_badge(cycle_r)}</div>
<div class="act-row"><span class="act-name">&#x1F97E; Wędrówki</span>{rating_badge(hike_r, hike_note)}</div>
<div class="act-row"><span class="act-name">&#x1F4F7; Fotografia</span>{rating_badge(photo_r)}</div>
<div class="act-row"><span class="act-name">&#x1F697; Jazda samochodem</span>{rating_badge(drive_r, drive_note)}</div>
"""
...
if today_rain_max >= 60:
    ...
    if today_max_t >= 25:
        ...
        lawn_reason = f"Gorący dzień ({today_max_t}&#xB0;C), brak opadów &#x2014; podlewaj wcześnie rano."
    elif today_max_t >= 15:
...
def describe_day(di):
    ...
    if has_rain and max_w >= 20:
        return f"{name} przynosi deszcz i silniejszy wiatr (do {max_w} km/h, {max_rain}% szans na opady)"
    if has_rain:
        return f"{name} z przelotnymi opadami, szczytowo {max_rain}% szans"
    return f"{name} suchy i spokojny, temperatura do {max_t_val}&#xB0;C"
...
def build_day_block(di):
    ...
    for pi, part in enumerate(PARTS):
      ...
      <div class="day-row{first}">
        {day_cell}
        <div class="part">{_PART_CAP[part]}</div>
...
cur_desc  = translate_desc(cur["weatherDesc"][0]["value"].strip())
...
<div class="card">
  <div class="header">
    <div class="hdr-left">
      ...
      <div>
        ...
        <div class="hdr-feels">Odczuwalna {cur_feels}&#xB0;C</div>
      </div>
      <div class="hdr-mid">
        <div class="hdr-location">&#x1F4CD; Bestwinka, Polska</div>
        <div class="hdr-desc">{cur_desc}</div>
        <div class="hdr-wind">&#x1F4A8; Wiatr {cur_wind} km/h</div>
      </div>
    </div>
    <div class="hdr-right">
      <div class="conf-title">Pewność prognozy</div>
  ...
  <div class="insights">
    <div class="section-title">Szybki przegląd</div>
    <div class="insights-grid">
      <div><div class="ins-icon">&#x1F302;</div><div class="ins-label">Ryzyko deszczu</div><div class="ins-val">{ins_rain}</div></div>
      <div><div class="ins-icon">&#x1F32C;&#xFE0F;</div><div class="ins-label">Wiatr</div><div class="ins-val">{ins_wind}</div></div>
      <div><div class="ins-icon">&#x1F324;&#xFE0F;</div><div class="ins-label">Najlepsza pora</div><div class="ins-val">{ins_best}</div></div>
      <div><div class="ins-icon">&#x1F321;&#xFE0F;</div><div class="ins-label">Komfort</div><div class="ins-val">{ins_comfort}</div></div>
      <div><div class="ins-icon">&#x26A0;&#xFE0F;</div><div class="ins-label">Uwaga</div><div class="ins-val">{ins_watch}</div></div>
    </div>
  </div>
  ...
  <div class="bottom4">
    <div class="bsec">
      <div class="bsec-title">Najlepsza i najgorsza pora</div>
      <div class="bw-item"><div class="bw-head g">&#x2705; NAJLEPSZA</div><div class="bw-time">{best_time}</div><div class="bw-why">{best_why}</div></div>
      <div class="bw-item"><div class="bw-head r">&#x274C; NAJGORSZA</div><div class="bw-time">{worst_time}</div><div class="bw-why">{worst_why}</div></div>
    </div>
    <div class="bsec">
      <div class="bsec-title">Co za&#x142;o&#x17C;y&#x107;</div>
      {wear_rows}
    </div>
    <div class="bsec">
      <div class="bsec-title">Aktywno&#x15B;ci</div>
      {act_rows}
    </div>
    <div class="bsec">
      <div class="bsec-title">Trend temperatury</div>
      {trend_svg}
    </div>
  </div>
  ...
  <div class="story">
    ...
    <div>
      <div class="story-label">Historia pogody</div>
...
print(f"index.html written — {cur_temp}°C {cur_desc}, pewność {conf_pct}%")
